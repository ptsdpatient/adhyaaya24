...
async function _sendEmail(to, subject, html, text, from = "Registrations @ Adhyaaya'23 <noreply.adhyaaya.gcoen@gmail.com>") {
...
async function sendEmail(registration_id) {
  const { data: registration, error } = await supabase.from("registrations").select("*").eq("id", registration_id).single();
  if (error) {
    console.error(error);
    return;
  }
  ...
  const html = getEmailHTML({
    ...
    date: event?.start_date.toLocaleDateString("en-IN", {
      weekday: "long",
      year: "numeric",
      month: "long",
      day: "numeric"
    ...
    receipt: `${"https://www.adhyaaya.org"}/pg/${registration.id}/success`
  });
